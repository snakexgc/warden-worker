...
async fn validate_membership_policies(
    ...
    let required_2fa: Option<i32> = db
        ...
        .await
        .map_err(|_| AppError::Database)?;
    if required_2fa.is_some() && !crate::two_factor::is_any_enabled(db, &member.user_id).await? {
...
async fn recover_account_impl(
    ...
    notify::notify_background(
        &state.ctx,
        state.env.clone(),
        crate::notify::NotifyEvent::PasswordChange,
...
pub async fn update_reset_password_enrollment(
    ...
    if reset_password_key.is_some() {
        crate::handlers::accounts::validate_password_or_otp(
            &db,
            &claims.sub,
            &crate::handlers::accounts::SecretVerificationRequest {
