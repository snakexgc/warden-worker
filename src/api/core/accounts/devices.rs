...
pub async fn post_auth_request(
    ...
    state.ctx.wait_until(async move {
        if let Err(e) =
            crate::api::notifications::publish_auth_request(&env, &user_id, &notify_request_id)
                .await
        {
            log::warn!("publish auth request realtime failed: {e}");
        }
        if let Err(e) =
            crate::extensions::notify::publish_auth_request(&env, &user_id, &notify_request_id)
                .await
        {
...
pub async fn put_auth_request(
    ...
    if payload.request_approved {
        ...
        state.ctx.wait_until(async move {
            if let Err(e) = crate::api::notifications::publish_auth_response(&env, &uid, &rid).await
            {
                log::warn!("publish auth response realtime failed: {e}");
            }
            if let Err(e) = crate::extensions::notify::publish_auth_response(&env, &uid, &rid).await
            {
