...
pub struct OrganizationImportRequest {
    ciphers: Vec<crate::db::models::cipher::CipherRequestData>,
...
pub async fn import_organization(
    ...
    if !crate::api::core::organizations::organizations_enabled(&state.env) {
    ...
    let member =
        crate::api::core::organizations::load_membership(&db, &claims.sub, &query.organization_id)
            .await?;
    crate::api::core::organizations::require_confirmed(&member)?;
    ...
    for import_collection in payload.collections {
        let name = crate::api::core::organizations::validate_name(
            &import_collection.name,
            "Collection name",
        )?;
        let collection_id = if let Some(collection_id) = import_collection.id {
            let collection = crate::api::core::organizations::load_collection(
                &db,
                &query.organization_id,
                &collection_id,
            )
            .await?;
            let (read_only, _, manage) = crate::api::core::organizations::collection_permissions(
                &db,
                &member,
                &collection.id,
            )
    ...
    let effective_full_access =
        crate::api::core::organizations::member_has_full_access(&db, &member).await?;
    ...
    crate::api::core::ciphers::finish_organization_batch_mutation(
        &db,
        &state,
        &query.organization_id,
        &claims.sub,
