        ...
        match get_user_ua_history(user_id, env).await {
            Ok(history) => {
                ...
                if let Err(e) = update_user_ua_history(user_id, env, ua).await {
                    log::warn!(target: crate::worker_runtime::logging::targets::NOTIFY, "update ua history failed user_id={} err={:?}", user_id, e);
                }
            }
            Err(e) => {
                log::warn!(target: crate::worker_runtime::logging::targets::NOTIFY, "get ua history failed user_id={} err={:?}", user_id, e);
            }
        }
    }
}
